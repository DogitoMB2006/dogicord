// src/components/server/ChannelEditor.tsx
...
interface ChannelEditorProps {
 ...
 onSave: (nameOrUpdates: string | Partial<Channel>, type?: 'text' | 'voice', categoryId?: string, permissions?: ChannelPermission[]) => Promise<void>
 onCancel: () => void
...
export default function ChannelEditor({
 ...
 const getPermissionDescription = (permission: ChannelPermissionType): string => {
   const descriptions: Record<ChannelPermissionType, string> = {
     view_channel: 'Allows members to view this channel',
     send_messages: 'Allows members to send messages in this channel',
   ...
   <div className="space-y-6">
     ...
     <div className="space-y-4">
                   ...
                   return (
                     <div key={permission} className="flex items-center justify-between">
                       <div className="flex-1">
                         <span className={`text-gray-300 ${isMobile ? 'text-sm' : 'text-base'}`}>
                           {CHANNEL_PERMISSIONS.find(p => p.id === permission)?.name}
                         </span>
       ...
       <div className="flex space-x-3 pt-4">
         ...
         {isEditing && onDelete && hasPermission('manage_channels') && (
           <button
